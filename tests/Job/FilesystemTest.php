...
final class FilesystemTest extends TestCase
{
    ...
    /**
     * Each directory keeps the mode its source had, rather than a constant.
     * The modes below are deliberately different from each other and from the
     * old hardcoded 0755, so a copy that ignored the source would fail here.
     */
    ...
    /**
     * moveDir() picks rename or copy on the mount layout alone, so the two
     * must leave the same modes behind -- otherwise a site's permissions would
     * depend on how the operator mounted the volumes.
     */
    ...
    public function testApplyPermissionsSetsTheWholeTree(): void
    {
        if (\function_exists('posix_geteuid') && posix_geteuid() === 0) {
            self::markTestSkipped('running as root: permission bits are not enforced');
        }
        ...
        Filesystem::applyPermissions($this->root . '/tree', 0o755, 0o644);
    ...
    /**
     * chmod() and chown() act on a symlink's target, so following one would
     * change permissions on something outside the tree entirely.
     */
    public function testApplyPermissionsSkipsSymlinksRatherThanFollowingThem(): void
    {
        if (\function_exists('posix_geteuid') && posix_geteuid() === 0) {
            self::markTestSkipped('running as root: permission bits are not enforced');
        }
    ...
    public function testApplyPermissionsRefusesAPathThatIsNotADirectory(): void
    {
        ...
        Filesystem::applyPermissions($this->root . '/afile', 0o755, 0o644);
    }

    /**
     * A wrong owner is a 403 on every page, so it fails the build rather than
     * warning. "nosuchuser!" cannot resolve, which is the same failure an
     * unprivileged worker would hit.
     */
    public function testApplyPermissionsThrowsWhenTheOwnerCannotBeSet(): void
    {
        $this->givenTree($this->root . '/tree');
        ...
        Filesystem::applyPermissions($this->root . '/tree', 0o755, 0o644, 'nosuchuser!');
    }
