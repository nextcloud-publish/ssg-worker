...
final class JobWorkspace
{
    ...
    public function publish(string $staticSiteId, string $buildId, string $slug): void
    {
        ...
        // The whole tree, not just its root. output/ arrives at 0750, and any
        // attachment carried out of the tarball keeps whatever mode the
        // archive gave it -- left alone, either is a 403 on a page or an image
        // that no unit test would catch. Owner and group are left to the
        // deployment; pass them here if the serving uid needs them.
        Filesystem::applyPermissions($staging, self::PUBLISHED_DIR_MODE, self::PUBLISHED_FILE_MODE);
